...
def post(post_id):
    ...
    if post is None:
        ...
        title = post['title']
        logger_msg('Article "%(title)s" retrieved!' % {"title": title})
...
@app.route('/about')
def about():
    logger_msg('The "About Us" page is retrieved!')
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            connection.commit()
            connection.close()
            logger_msg('A new article "%(title)s" is created!' %
                       {"title": title})
            return redirect(url_for('index'))

    return render_template('create.html')

# Solve favicon.icon 404 problem
